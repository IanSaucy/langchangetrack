...
class Embeddings(object):
    ...
    def __init__(self, lang, filename=None, vectors=None, words=None):
        ...
        if words:
            if len(set(words)) == len(words):
                ...
                self.word_id = {'{}_{}'.format(w, i): i for i, w in enumerate(words)}
    ...
    def __most_frequent(self, n, start=0):
        return [x for x, y in sorted(iter(self.word_id.items()), key=lambda x_y: x_y[1])[start:n]]
    ...
    def least_frequent_n(self, n):
        return [x for x, y in sorted(iter(self.word_id.items()),
                                     key=lambda x_y1: x_y1[1], reverse=True)[:n]]
    ...
    def sorted_words(word_id):
        return sorted(iter(word_id.items()), key=lambda x_y2: x_y2[1])
    ...
    def split(self, mapping, ignore_exact=True):
        ...
        new_mapping.map = dict(list(zip(source.words, target.words)))
...
class Evaluator(object):
    ...
    def precision_at_k_l2(self, test_pairs):
        ...
        for s, t in test_pairs:
            ...
            line = "{: <20}{:<20}{:<50}".format(s, t, ' '.join(t_words))
    ...
    def evaluate(self, mapping, operation, training_segment, test_segment):
        ...
        if set(s_train.words).intersection(set(s_test.words)):
            print ("Train and test words are overlapping")
